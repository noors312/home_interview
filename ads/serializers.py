...
class GetAdRequestSerializer(serializers.Serializer):
    SDK_version = serializers.CharField(allow_blank=True, allow_null=True, required=False, source='sdk_version')
    ...
    country_code = serializers.CharField(allow_blank=True, allow_null=True, required=False)

    def validate(self, attrs):
        super(GetAdRequestSerializer, self).validate(attrs)
        if not any([attrs.get('sdk_version'), attrs.get('username')]):
            raise serializers.ValidationError('You should provide either SDKVersion or username')
        return attrs
...
class AdStatsSerializer(serializers.Serializer):
    impression_per_client = serializers.DecimalField(decimal_places=2, max_digits=15, required=False)
    ad_per_client = serializers.DecimalField(decimal_places=2, max_digits=15, required=False)
